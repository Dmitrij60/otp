...
class Loader
{
    ...
    public static function loadFiles($userId)
    {
        ...
        $path = $_SERVER['DOCUMENT_ROOT'] . "/upload/documents/{$userId}/";
        if (isset($_FILES[$input_name])) {
            ...
            foreach ($files as $file) {
                ...
                if (!empty($file['error']) || empty($file['tmp_name'])) {
                    switch (@$file['error']) {
                        ...
                        case 4:
                            ...
                            break;
                        case 6:
                            $error = 'Файл не был загружен - отсутствует временная директория.';
                            break;
                        case 7:
                ...
                if (!empty($success)) {
                    ...
                    echo '<p class="download-status">' . $success . '</p><br>';


                } else {
                    echo '<p class="download-status">' . $error . '</p><br>';
                }
            }
